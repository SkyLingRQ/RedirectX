...
func redirectX(URL string, wg *sync.WaitGroup, sem chan struct{}, useragent string) {
	defer wg.Done()
	...
	if redirectURL != nil && strings.HasPrefix(redirectURL.String(), "https://www.google.com") {
...
func main() {
	...
	file := flag.String("file", "redirect.txt", "Archivo con las URLs a escanear.")
	semaphore := flag.Int("semaphore", 50, "Implementar semaforo personalizado.")
	...
	if err != nil {
		fmt.Println("[x] Error reading file " + *file)
		return
	}
	defer contentFile.Close()
	scannerBufio := bufio.NewScanner(contentFile)

	for scannerBufio.Scan() {
		URL := scannerBufio.Text()
		...
		for key := range qs {
			qs.Set(key, "https://www.google.com")
		}

		urlParsed.RawQuery = qs.Encode()
		useragentRandom := userAgents[rand.Intn(len(userAgents))]
		wg.Add(1)
		go redirectX(urlParsed.String(), &wg, sem, useragentRandom)
	}
